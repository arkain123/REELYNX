...
int kol_wozeros(int array[][], int elemi, int elemj, int out) {			//kol_without_zeros
	for (int i = 0; i < elemi; i++) {					//Ищем количество строк без нулей
		int zero = 0;
		for (int j = 0; j < elemj; j++) {
			if (array[i][j] == 0) {
				zero++;
			}
		}
...
int main() {    
	...
	*/
	...
	kol_strings = kol_wozeros(m, 10, 10, kol_strings);
